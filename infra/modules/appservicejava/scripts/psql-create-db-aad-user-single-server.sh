...
echo "PostgreSQL Server ${PSQL_SERVER_NAME} creating AADUSER role in database ${DATABASE_NAME} on ${DATABASE_FQDN}..."
...
psql "host=${DATABASE_FQDN} user=${USER_NAME}@${PSQL_SERVER_NAME} dbname=${DATABASE_NAME} port=5432 password=${RDBMS_ACCESS_TOKEN} sslmode=require" <<EOF
